...
def test_create_chat_message(auth_client, user):
    ...
    assert chat_message.user_input == "Hello, AI!"
...
def test_get_chat_messages(auth_client, user):
    ...
    MhaiChat.objects.create(
        user=user, user_input="First message", status="completed"
    )
    MhaiChat.objects.create(
        user=user, user_input="Second message", status="completed"
    )
...
def test_filter_chat_messages_since_id(auth_client, user):
    ...
    assert last_message["user_input"] == message2.user_input
